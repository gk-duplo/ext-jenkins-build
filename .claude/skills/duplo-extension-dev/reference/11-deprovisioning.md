...
**Frontend: the Delete action.** Ship a Delete row action (list) + `#actions` item (view) — worked in
...
`DeleteConfirmationModalService.openGeneric(...)` wrapped in try/catch with a `window.confirm` fallback (host
modal services can throw inside a remote), and any failure surfaced in the page's error alert — never
swallowed.
