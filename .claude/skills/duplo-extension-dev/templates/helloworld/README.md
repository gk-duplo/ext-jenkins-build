...
The Result panel is hand-written markup. The OPTIONAL declarative renderer (simple/static results only,
vendored from `samples/helloworld`) is covered by reference/09-result-templates.md; rich tabbed results
(Overview | Logs | …) by reference/17-custom-result-views.md.
...
- A per-extension Native-Federation remote that shares the host's Angular 22 / RxJS singletons, reaches
  ...
  view-with-sidecards) so it looks native.
