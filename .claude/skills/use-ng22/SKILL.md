...
`src/app/result-template/` (present only if you chose the declarative renderer), a verbatim copy of host code
that stays on the old shape until the lib exports it. Everything else — including custom result-view panel
components (reference/17) — IS your code and must pass this gate. Point `$OWN` at your own directories.
